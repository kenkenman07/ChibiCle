...
Uses GPS trajectory + OSM road geometry to determine cyclist's side.
"""
...
def _is_wrong_side(
    ...
    # Japan: same direction → should be LEFT, opposite → should be RIGHT
    wrong = is_on_left != same_direction

    # One-way: opposite direction is always a violation
...
class RightSideDetector(GpsViolationDetector):
    ...
    async def detect(
        ...
        roads = await self._source.get_roads(center_lat, center_lng)
        if not roads:
            return []
